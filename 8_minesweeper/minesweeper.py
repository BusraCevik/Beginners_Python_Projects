...
class Board:
    def __init__(self, dim_size, num_bombs):
        #lets keep track of these parameters, they will be helpfull later
        ...
        self.assign_values_to_board()#to figure it out the bomb nums around us
    ...
    def make_new_board(self):
        ...
        #we should construct the list of lists here(represents 2d board)
        ...
        while bombs_planted < self.num_bombs:
            ...
            row = loc // self.dim_size # bölüm
            col = loc % self.dim_size #mod

            if board[row][col] == '*':
                #this means we have actually planted a bomb there already sk keep going
                continue
    ...
    def get_num_neighboring_bombs(self, row,col):
        ...
        num_neighboring_bumbs = 0
        for r in range(max(0,row-1), min(self.dim_size-1, row+1)+1):
            for c in range(max(0, col-1),min(self.dim_size - 1, col+1)+1):
                if r == row and c == col:
                    #our original loc, dont check
                    continue
                if self.board[r][c] == '*':
                    num_neighboring_bumbs += 1

        return num_neighboring_bumbs
    ...
    def dig(self, row, col):
        ...
        #a few scenarios
        #hit a bomb->game over //false
        #dig at location with neighboring bomb->finish dig(numbers came out true)
        #dig at location with no neighboring bombs -> recursively dig neighbors(empty space also true)
        ...
        if self.board[row][col] == '*':
            ...
            return True
        #self.board[row][col] == 0 // no bomb or no numbers it means this space is equal to none aka zero
        for r in range(max(0,row-1), min(self.dim_size-1, row+1)+1):
            for c in range(max(0, col-1),min(self.dim_size - 1, col+1)+1):
                if(r, c) in self.dug:
                    continue #dont dig where you already dug
                self.dig(r,c)
        #if our initial dig did not hit a bomb, we should not hit a bomb here
        return True
